...
public import wasmer.bindings.cwasmer;
...
class Module : Handle!wasm_module_t
{
    ...
    ubyte[] serialize()
    {
        ...
        if (dataVec.size == 0 || dataVec.data == null)
            return null;
...
class Instance : Handle!wasm_instance_t
{
    ...
    this(Store store, Module module_, Extern[] imports = [])
    {
        ...
        auto importsVecElements = cast(wasm_extern_t**) imports.map!(
                import_ => import_.handle).array.ptr;
        wasm_extern_vec_new(&importObject, imports.length, importsVecElements);

        super(wasm_instance_new(cast(wasm_store_t*) store.handle,
                cast(wasm_module_t*) module_.handle, &importObject, null));
...
class Value : Handle!wasm_val_t
{
    ...
    {
        this.kind = kind;
        super(new wasm_val_t(cast(wasm_valkind_t) kind));
    }
    ///
    this(int value)
    {
...
version (unittest)
{
    const string wat_hostref_module = "(module" ~ ""
...
extern (C) alias Callback = wasm_trap_t* function(const wasm_val_vec_t* arguments,
        wasm_val_vec_t* results);
/// A function to be called from WASM code. Includes an environment variable.
extern (C) alias CallbackWithEnv = wasm_trap_t* function(void* env,
        const wasm_val_vec_t* arguments, wasm_val_vec_t* results);
...
private enum bool isInt32(T) = __traits(isSame, T, bool) || __traits(isSame, T,
            int) || __traits(isSame, T, uint);
...
wasm_functype_t* functype(Func)(Func _) if (isCallableAsFunction!Func)
{
    ...
    else static if (isFloat64!(ReturnType!Func))
    {
        ...
        static assert(0, "Unsupported function return type!");
...
private extern (C) wasm_trap_t* callbackWithDelegate(void* env,
        const wasm_val_vec_t* arguments, wasm_val_vec_t* results)
{
    import std.algorithm : map;
    import std.array : array;
    ...
    try
    {
        ...
        Value[] argumentsManaged = arguments.data[0 .. arguments.size].map!(
                arg => Value.from(arg)).array;
...
template isCallableAsFunction(T...) if (T.length == 1 && isCallable!T)
{
    ...
    static assert(TParams.length > 0,
            "The delegate must at least have one argument of type `Module`");
    static if (TParams.length == 1 && !isModule!(TParams[0]))
    {
        static assert(0, "The first parameter in the method must be of type `Module`");
    }
    enum bool isCallableAsFunction = allSatisfy!(templateOr!(isBoolean,
                isIntegral, isFloatingPoint, isModule), TParams);
}
...
@safe unittest
{
    interface I
    {
        void run(Module) const;
    }

    struct S
    {
        static void opCall(Module, bool, int)
        {
        }
    }
    ...
    class C
    {
        void opCall(double, float, Engine)
        {
        }
    }
...
class Function : Handle!wasm_func_t
{
    ...
    this(Func)(Store store, Module module_, Func callback)
            if (isCallableAsFunction!Func)
    {
        this(store, callback.functype, &callbackWithDelegate, new CallbackContext(module_,
                ...
                template diagnosticNameOf(T)
                {
                    static if (paramName!T != "")
                        ...
                        enum string name = "";
                    enum string diagnosticNameOf = "parameter " ~ text(
                        indexOf!T + 1) ~ name ~ " of type `" ~ fullyQualifiedName!(Unqual!T) ~ "`";
                }
                ...
                static foreach (Param; FuncParams)
                {
                    ...
                    else static if (isInt32!(Unqual!Param))
                    {
                        ...
                        params[indexOf!Param] = arguments[indexOf!Param - 1].value.of.i32.to!(
                        Unqual!Param);
                    }
                    else static if (isInt64!(Unqual!Param))
                    {
                        assert(arguments[indexOf!Param - 1].kind == wasm_valkind_enum.WASM_I64);
                        params[indexOf!Param] = arguments[indexOf!Param - 1].value.of.i64.to!(
                        Unqual!Param);
                    }
                    ...
                    {
                        static assert(0,
                        "Could not apply " ~ diagnosticNameOf!Param ~ " to callback delegate");
                    }
                }
                ...
                else static if (__traits(isSame, ReturnType!Func, Value[]))
                {
                    ...
                    assert(cbResults.length >= results.length,
                    "Could not apply return values of callback delegate, expected " ~ text(
                    results.length) ~ " results, but received " ~ text(cbResults.length));
    ...
    Extern asExtern(string name = "") @property const
    {
        ...
        return new Extern(wasm_func_as_extern(handle),
                wasm_exporttype_new(&nameVec, wasm_functype_as_externtype(type)));
    }
    ...
    bool call(Value[] arguments, out Value[] results)
    {
        ...
        if (trap !is null)
            throw new Exception(new Trap(trap).message);
...
version (unittest)
{
    const string wat_callback_module = "(module"
        ...
        ~ "    (i32.add" ~ "      (call $print (i32.add (local.get $x) (local.get $y)))"
        ~ "      (call $closure)" ~ "    )" ~ "  )" ~ ")";

    package extern (C) wasm_trap_t* closure(void* env,
...
unittest
{
    ...
    auto imports = [
        ...
        new Function(store, wasm_functype_new_0_1(wasm_valtype_new_i32()),
                &closure, &i).asExtern
    ];
...
version (unittest)
{
    const string wat_trap_module = "(module" ~ "  (func $callback (import \"\" \"callback\") (result i32))"
...
unittest
{
    ...
    auto imports = [
        new Function(store, module_, wasm_functype_new_0_1(wasm_valtype_new_i32()),
                fail.toDelegate).asExtern
    ];
    ...
    assert(instance.exports[0].name == "callback" && callbackFunc.valid,
            "Failed to get the `callback` function!");

    Value[] results;
    assert(collectExceptionMsg!Exception(callbackFunc.call(results)) == "callback abort",
